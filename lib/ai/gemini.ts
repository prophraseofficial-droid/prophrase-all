...
export async function generateOutcomeAssistantWithGemini({
  ...
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), OUTCOME_GEMINI_TIMEOUT_MS);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new AiProviderError({
        message: `GEMINI_OUTCOME_PROVIDER_${response.status}${
          providerStatus ? `_${providerStatus}` : ""
        }${providerMessage ? `: ${providerMessage}` : ""}`,
        providerStatus,
        statusCode: response.status,
    ...
    if (!result) {
      throw new Error("GEMINI_EMPTY_OUTCOME_RESPONSE");
    }
